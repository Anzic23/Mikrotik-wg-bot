...
class MikrotikAPI:
    def __init__(self):
        ...
        min_ip_str = os.getenv("WG_POOL_MIN", "")
        self.min_ip = ipaddress.ip_address(min_ip_str) if min_ip_str else None
    ...
    def list_peers(self):
        r = requests.get(self._url("/interface/wireguard/peers"), auth=self.auth, verify=self.verify_ssl, timeout=15)
        r.raise_for_status()
        return r.json()
    ...
    def next_free_ip(self) -> str:
        ...
        for h in self.pool.hosts():
            if self.min_ip and h < self.min_ip:
                continue
            if h in used:
                continue
            return f"{h}/32"
    ...
    def generate_keys(self):
        ...
        pub = subprocess.run(["wg", "pubkey"], input=priv, stdout=subprocess.PIPE, check=True).stdout.strip()
        psk = subprocess.check_output(["wg", "genpsk"]).strip()
        return priv.decode(), pub.decode(), psk.decode()

    def get_server_public_key(self):
        r = requests.get(self._url("/interface/wireguard"), auth=self.auth, verify=self.verify_ssl, timeout=15)
        r.raise_for_status()
        items = r.json()
        for it in items:
    ...
    def add_peer(self, name: str) -> str:
        """Создаёт peer на MikroTik и возвращает готовый клиентский конфиг (скелет, без профиля AllowedIPs)."""
        ...
        r = requests.post(self._url("/interface/wireguard/peers/add"), auth=self.auth, json=payload, verify=self.verify_ssl, timeout=15)
        r.raise_for_status()
        ...
        config = (
            ...
            # AllowedIPs мы подменяем в bot.py согласно выбранному профилю
